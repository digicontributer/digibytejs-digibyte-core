...
- [Building DigiByte Core](#building-digibyte-core)
...
Clone the git repositories for digibyte and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/digibyte-project/digibyte
...
in the digibyte repository under 'Fetch and create inputs' to install sources which require
...
To build DigiByte Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the digibyte repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/digibyte/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/digibyte-project/digibyte
...
For example:
```bash
URL=https://github.com/thrasher-/digibyte.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit digibyte=${COMMIT} --url digibyte=${URL} ../digibyte/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit digibyte=${COMMIT} --url digibyte=${URL} ../digibyte/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit digibyte=${COMMIT} --url digibyte=${URL} ../digibyte/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the digibyte git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../digibyte/contrib/gitian-descriptors/*|sort|uniq )
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/digibyte-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/digibyte-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/digibyte-osx-build.assert
```
...
[gitian.sigs.ltc](https://github.com/digibyte-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
