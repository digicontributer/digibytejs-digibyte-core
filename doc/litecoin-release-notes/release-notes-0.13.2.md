DigiByte Core version 0.13.2 is now available from:

  <https://download.digibyte.org/digibyte-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/digibyte-project/digibyte/issues>
...
updates will be released anymore. Without security updates, using a digibyte
...
ECDSA signatures inside DigiByte transactions now use validation using
...
Previous versions of DigiByte Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of DigiByte Core was to raise the default minimum
relay fee.

DigiByte Core 0.13.2 will have a strict maximum size on the mempool. The
...
DigiByte Core 0.13.2 also introduces new default policy limits on the length and
...
DigiByte Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. DigiByte Core will relay transactions with
...
entirely in DigiByte Core 0.13, and it is at this time undecided whether the
...
DigiByte Core automatically creates a hidden service to listen on, without
manual configuration. DigiByte Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if DigiByte Core is listening, and
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals DigiByte
Core to use floating fees. By default, DigiByte Core will use floating
fees.
...
(default: `0.0002` DGB/kB).

At all times, DigiByte Core will cap fees at `-maxtxfee=<x>` (default:
0.10) DGB.
Furthermore, DigiByte Core will never create transactions paying less than
...
`digibyte.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
- Adding `dbcache=100` in digibyte.conf
...
digibyte-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/digibyte-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The DigiByte Core code base has
...
- `digibyte-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `digibyte-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the DigiByte scripting system that in combination with
...
DigiByte transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of DigiByte Core.
...
The two phases are distinct in the DigiByte-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming DigiByte wallet
  software more difficult and which seriously complicates the design of smart
  contracts for DigiByte.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each DigiByte block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** DigiByte addresses (both P2PKH addresses
  ...
  believed to be provided by DigiByte's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of DigiByte's current ledger (called the UTXO set),
  ...
  note that DigiByte Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  DigiByte users to individually opt-in to almost any change in the DigiByte
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and DigiByte Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the DigiByte
client to DigiByte Core 0.13.2 to signal for segwit support. When a
...
Since DigiByte Core 0.10.0, nodes have defaulted to only relaying and
...
- `digibyte-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `digibyte-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
