...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s digibyte-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  digibyte-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded digibyte transaction") + "\n" +
              "  digibyte-tx [options] -create [commands]   " + _("Create hex-encoded digibyte transaction") + "\n" +
              "\n";
